...
from rover_pkg import ArmData
from rover_pkg import FastAnalogData
from rover_pkg import SlowAnalogData
...
def slow_sensor_sweep():
	...
	SlowPublisher = rospy.Publisher('slow_sensor_data_stream', 'slowSensorData', queue_size=1)
...
def fast_sensor_sweep():	
	FastPublisher = rospy.Publisher('fast_sensor_data_stream', 'fastSensorData', queue_size=1)
	fastdata = FastAnalogData()
	fastdata.bd_sig = fast_read_p0.value	
	fastdata.ad_sig = fast_read_p1.value	
	fastdata.wpd_sig = fast_read_p2.value	
	fastdata.table_sig = fast_read_p3.value
	FastPublisher.publish(fastdata)

def init():
	rospy.init_node('sensor_read_node', anonymous=True)
	rospy.loginfo("Sensor Read node started")
