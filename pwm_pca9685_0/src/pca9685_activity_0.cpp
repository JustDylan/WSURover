...
namespace pwm_pca9685_0 {

// ******** constructors ******** //

PCA9685Activity_0::PCA9685Activity_0(ros::NodeHandle &_nh, ros::NodeHandle &_nh_priv) :
...
bool PCA9685Activity_0::set(uint8_t channel, uint16_t value) {
...
bool PCA9685Activity_0::start() {

    if(!sub_command) sub_command = nh.subscribe("general_pwm_driver_stream", 1, &PCA9685Activity_0::onCommand, this);
...
bool PCA9685Activity_0::spinOnce() {
    ros::spinOnce();
    ros::Time time = ros::Time::now();
    uint64_t t = 1000 * (uint64_t)time.sec + (uint64_t)time.nsec / 1e6;
...
void PCA9685Activity_0::onCommand(const std_msgs::Int32MultiArrayPtr &msg) {
    ros::Time time = ros::Time::now();
    uint64_t t = 1000 * (uint64_t)time.sec + (uint64_t)time.nsec / 1e6;
